<?php
    class ModelExtensionModuleIeProTabMigrations extends ModelExtensionModuleIePro
    {
        ...
        public function get_fields() {
            $this->document->addStyle($this->api_url.'/opencart_admin/ext_ie_pro/css/tab_migrations.css?'.date('Ymdhis'));
            $this->document->addScript($this->api_url.'/opencart_admin/ext_ie_pro/js/tab_migrations.js?'.date('Ymdhis'));
            ...
            try {
                ...
                foreach ($post_data as $key => $value) {
                    if (strpos($key, 'exportcat_') !== false)
                        $categories[] = str_replace("exportcat_", "", $key);
                }
        ...
        public function migration_import() {
            ...
            try {
                ...
                $formats_allowed = array('xlsx', 'ods', 'xml');
            ...
            foreach ($data as $table_name => $table_fields) {
                foreach ($table_fields['data'] as $field_num => $fields) {
                    ...
                    foreach ($fields as $field_name => $value) {
                        if($this->file_type == 'xlsx' && strlen($value) > 32767)
