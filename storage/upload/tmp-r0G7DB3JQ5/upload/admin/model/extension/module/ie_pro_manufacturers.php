    ...
    class ModelExtensionModuleIeProManufacturers extends ModelExtensionModuleIePro {
        ...
        function get_columns_formatted($multilanguage) {
            $columns = array(
                ...
                'Name' => array('multilanguage' => $multilanguage && $this->is_ocstore, 'hidden_fields' => array('table' => $multilanguage && $this->is_ocstore ? 'manufacturer_description' : 'manufacturer', 'field' => 'name')),
            ...
            foreach ($export_format as $manufacturer_id => $names) {
                foreach ($names as $lang_id => $name) {
                    ...
                    if($continue) {
                        ...
                        $final_manufacturers[$name .(!$select_format ?  '_' . $lang_id : '')] = $manufacturer_id;
                    }
                }
            }
        ...
        public function create_manufacturers_from_product($file_data) {
            ...
            foreach ($file_data as $key => $product) {
                ...
                $manufacturer_name = array_key_exists($this->main_table, $product) && array_key_exists('manufacturer_id', $product[$this->main_table]) && !empty($product[$this->main_table]['manufacturer_id']) ? $product[$this->main_table]['manufacturer_id'] : '';
                if(!empty($manufacturer_name)) {
                    $manufacturer_name = $this->sanitize_value($manufacturer_name);
                    ...
                    if (!$found && $some_with_name) {
                        $data_temp = array('name' => array($this->default_language_id => $manufacturer_name));
        ...
        public function create_simple_manufacturer($data) {
                ...
                foreach ($data['name'] as $language_id => $name) {
                    if(!empty($name)) {
                        $sql = "INSERT INTO " . $this->escape_database_table_name('manufacturer_description') . " SET " .
                            ...
                            $this->escape_database_field('meta_title') . " = " . $this->escape_database_value($name) . ", " .
